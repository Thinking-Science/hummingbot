...
def get_leverage_by_trading_pair(self, trading_pair: str) -> int:
    trading_pair = 20
    return trading_pair
...
class LooterKing(ScriptStrategyBase):
    ...
    trading_pairs = ["DODO-BUSD", "PHB-BUSD", "XRP-BUSD", "AMB-BUSD", "FTM-BUSD", "GALA-BUSD", "TRX-BUSD", "DOGE-BUSD",
                     "AGIX-BUSD"]
    ...
    for trading_pair in trading_pairs:
        roulette_by_trading_pair[trading_pair] = {
            ...
            "game_over_usd": 6,
            "leverage": get_leverage_by_trading_pair(trading_pair),
    ...
    def on_tick(self):
        ...
        for trading_pair, roulette_info in self.roulette_by_trading_pair.items():
            ...
            if roulette_info['active_trading']:
                ...
                continue
            if len(roulette_info["active_executors"]) < self.max_roulettes and roulette_info["candles"].is_ready and \
                    roulette_info['candles_1m'].is_ready:
                ...
                if signal < self.short_threshold or signal > self.long_threshold:
                    bet = self.get_roulette_amount(trading_pair, roulette_info,
                                                   take_profit)  # sets ball number - roullete_id - amount+extraamount
                    position_side = PositionSide.LONG if signal > 0 else PositionSide.SHORT
                    price = self.get_entry_price(trading_pair, roulette_info,
                                                 position_side)  # gets best bid/ask price * buffer
                    self.logger().info(f"""
...
Amount: {bet} | Take Profit: {take_profit} | Stop Loss: {stop_loss}
    ...
    def set_cash_out_trigger(self, trading_pair, roulette_info):
        ...
        if roulette_info['cashing_out']:
            current_loosing_balls, current_roullete_loss_usd = self.calculate_loosing_balls(
                roulette_info['stored_executors'])
    ...
    def get_signal_std(self, trading_pair):
        ...
        return signal_value, min(std_pct_1m, std_pct)
    ...
    def format_status(self) -> str:
        ...
        for trading_pair, roulette_info in self.roulette_by_trading_pair.items():
            ...
            if len(roulette_info["stored_executors"]) > 0:
                ...
                win_roulettes = len([x for x in roulette_info["stored_executors"] if
                                     x.status == PositionExecutorStatus.CLOSED_BY_TAKE_PROFIT])
                ...
                for roullete_id, roullete_stat in mkt_roullete_stats:
                    ...
                    lasts_roulettes_resume += str(
                        f" | {roullete_stat['ball_numbers']}B{roullete_stat['net_pnl']:.4f} | ")
            ...
            if len(roulette_info["active_executors"]) > 0:
                for executor in reversed(roulette_info["active_executors"]):
                    if executor.status == PositionExecutorStatus.ORDER_PLACED:
                        ...
                        active_balls += 1
                    if Decimal(executor.amount) * Decimal(executor.entry_price) / Decimal(
                            roulette_info["leverage"]) > max_margin_reached:
                    ...
                    if executor.status == PositionExecutorStatus.ORDER_PLACED:
                        ...
                        stop_result += Decimal(executor.pnl_usd) - Decimal(executor.cum_fees) - Decimal(
                            self.taker_fee) * Decimal(executor.amount) * Decimal(current_price)
            mkt_activity_status += f" | {mkt_activity_status_aux}"
            results_per_mkt.insert(1,
...
Net result: {(pnl_usd - fees_cum_usd):.4f}\nMax Margin {max_margin_reached:.4f} | Runaway: {stop_result:.4f} (USD)""")
        ...
        total_results.insert(2,
                             f"Net result: {(realized_pnl_usd_global - fees_cum_usd_global):.4f} | {win_roulletes_global} WINS\n")
        total_results.insert(3,
                             f"Unrealized PNL: {unrealized_pnl_usd_global:.4f}U$D\nPlaying: {active_balls_global} - Order placed:{wating_enter_balls_global}\n")
        if lossing_balls_composition != '':
            total_results.insert(5,
                                 f"Losing: {current_loosing_balls_global} Balls | {current_roullete_loss_usd_global:.4f} U$D\n {lossing_balls_composition}")
        else:
            total_results.insert(5, "(NO LOOSING ROULETTES)\n")
        total_results.insert(5, f"Lost Roulletes: {lost_balls_global}")
        total_results.insert(6, f"Runaway: {stop_result_global:.4f} U$D")
        total_results.insert(7,
                             f"Stop net result: {(realized_pnl_usd_global - fees_cum_usd_global + stop_result_global):.4f} U$D")
        lines.append("\n### TIME FOR CASHOUT ###")
    ...
    def clean_and_store_executors(self):
        for trading_pair, roulette_info in self.roulette_by_trading_pair.items():
            ...
            waiting_executors = [x for x in roulette_info["active_executors"] if
                                 x.status == PositionExecutorStatus.ORDER_PLACED and x.position_config.timestamp + minutes * 60 <= self.current_timestamp]
            for executor_waiting_to_enter in waiting_executors:
                ...
                buffer_used = 1 + roulette_info["limit_order_price_buffer"] \
                    if executor_waiting_to_enter.side == PositionSide.SHORT else 1 - roulette_info["limit_order_price_buffer"]
        ...
        if current_roullete_loss_usd < 0:
            if abs(current_roullete_loss_usd) >= abs(game_over_usd):
                self.notify_hb_app_with_timestamp(
                    f"Game over! Max Loss reached {trading_pair}: {current_roullete_loss_usd:.4f} U$D")
